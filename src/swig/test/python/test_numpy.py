...
def test_Array2Diterator():
    a = FloatArray2D(IndexRange2D(Int2BasicCoordinate(
        (1, 3)), Int2BasicCoordinate((3, 9))))
...
def test_Array3Diterator():
    a = FloatArray3D(IndexRange3D(Int3BasicCoordinate(
        (1, 3, -1)), Int3BasicCoordinate((3, 9, 5))))
    for i1, i2 in zip(a.flat(), range(a.size_all())):
        i1 = i2
    np = stirextra.to_numpy(a)
    for i1, i2 in zip(a.flat(), np.flat):
